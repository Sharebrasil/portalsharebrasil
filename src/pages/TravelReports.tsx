...
const TravelReports = () => {
  ...
  const getReportPdfUrl = async (report?: any): Promise<string | null> => {
    try {
      ...
      const candidatePath = targetReport?.client_name ? `${toFolder(targetReport.client_name)}/${safeNum}.pdf` : null;
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        if (!error) {
          setClient((data || []).map((c: any) => ({ id: c.id, nome: c.company_name })));
        }
      } catch (e) {
        // ignore
      }
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        if (!error) setCrewMembers((data || []).map((t:any)=>t.full_name).filter(Boolean));
      } catch (e) { }
    })();
  }, []);
  ...
  const dbClientNames = (client || []).map((c:any) => String(c?.nome || '').trim()).filter(Boolean);
  const reportClientNames = Array.from(new Set((reports || []).map((r:any) => String(r?.client_name || '').trim()).filter(Boolean)));
  const historyClientNames = Array.from(new Set((history || []).map((h:any) => String(h?.cliente || '').trim()).filter(Boolean)));
  const allClientFolders = Array.from(new Set([
    ...dbClientNames,
    ...reportClientNames,
    ...historyClientNames,
  ].filter(Boolean))).sort((a, b) => String(a).localeCompare(String(b), 'pt-BR'));
  const visibleReports = (selectedClient ? (reports || []).filter((r:any) => r?.client_name === selectedClient) : (reports || []))
    .filter((r:any) => r?.status !== 'finalized');
  const uniqueCrewMembers = Array.from(new Set((crewMembers || []).filter(Boolean))).sort((a, b) => String(a).localeCompare(String(b), 'pt-BR'));
  const OUTRO_OPTION = '__OUTRO__';
  const [showSecondCrew, setShowSecondCrew] = useState<boolean>(false);
  const [isOutroCrew, setIsOutroCrew] = useState<boolean>(false);
  
  useEffect(() => {
    const t = currentReport?.crew_member_name || '';
    if (t && !uniqueCrewMembers.includes(t)) setIsOutroCrew(true);
    else setIsOutroCrew(false);
  }, [currentReport?.crew_member_name, uniqueCrewMembers.join('|')]);
  
  const crewSelectValue = isOutroCrew
    ? OUTRO_OPTION
    : (uniqueCrewMembers.includes(currentReport?.crew_member_name || '') ? (currentReport?.crew_member_name || '') : '');

  const [isOutroCrew2, setIsOutroCrew2] = useState<boolean>(false);
  useEffect(() => {
    const t2 = currentReport?.crew_member_name_2 || '';
    if (t2 && !uniqueCrewMembers.includes(t2)) setIsOutroCrew2(true);
    else setIsOutroCrew2(false);
  }, [currentReport?.crew_member_name_2, uniqueCrewMembers.join('|')]);
  
  const crew2SelectValue = isOutroCrew2
    ? OUTRO_OPTION
    : (uniqueCrewMembers.includes(currentReport?.crew_member_name_2 || '') ? (currentReport?.crew_member_name_2 || '') : '');
  ...
  const deleteCreatedReport = async (report: any) => {
    ...
    if (!ok) return;
    try {
      const folder = toFolder(report.client_name);
  ...
  const generateReportNumberLocal = (clientName) => {
    const yyyy = String(new Date().getFullYear());
    if (!clientName) return `XXX001/${yyyy}`;

    const clientLetters = clientName.substring(0, 3).toUpperCase().padEnd(3, 'X');

    const maxNumber = (reports || [])
      .filter((report:any) => report.client_name === clientName)
    ...
    return `${clientLetters}${String(maxNumber + 1).padStart(3, '0')}/${yyyy}`;
  };

  const allocateReportNumber = async (clientName: string) => {
    try {
      ...
      if (error) {
        ...
        return generateReportNumberLocal(clientName);
      }
  ...
  const generateReportNumber = (clientName: string) => generateReportNumberLocal(clientName);

  const createNewReport = () => {
    const lastCreated = (()=>{ try { return localStorage.getItem('last_created_crew_name') || ''; } catch { return ''; } })();
    const newReport = {
      numero: '',
      client_name: '',
      aircraft_registration: '',
      crew_member_name: lastCreated || '',
      crew_member_name_2: '',
      destination: '',
      start_date: toInputDateLocal(new Date()),
      end_date: toInputDateLocal(new Date()),
    ...
    if (lastCreated) { try { localStorage.removeItem('last_created_crew_name'); } catch {} }
  ...
  const calculateTotals = (despesas) => {
    ...
    despesas.forEach(despesa => {
      ...
      switch (despesa.categoria) {
        case 'Combustível':
          totals.total_fuel += valor;
          break;
        case 'Hospedagem':
          totals.total_lodging += valor;
          break;
        case 'Alimentação':
          totals.total_food += valor;
          break;
        case 'Transporte':
          totals.total_transport += valor;
          break;
        default:
          totals.total_other += valor;
      }

      switch (despesa.pago_por) {
        case 'Tripulante':
          totals.total_crew += valor;
          break;
        case 'Cliente':
          totals.total_client += valor;
          break;
        case 'ShareBrasil':
          totals.total_sharebrasil += valor;
          break;
      }
    });

    const total_amount = totals.total_crew + totals.total_client + totals.total_sharebrasil;
  ...
  const calculateDays = () => {
    if (currentReport?.start_date && currentReport?.end_date) {
      const inicio = new Date(currentReport.start_date);
      const fim = new Date(currentReport.end_date);
  ...
  const saveReport = async (status = 'draft') => {
    ...
    if (status === 'finalized') {
      ...
      if (!updatedReport.client_name) missing.push('Cliente');
      if (!updatedReport.aircraft_registration) missing.push('Aeronave');
      if (!updatedReport.crew_member_name) missing.push('Tripulante');
      if (!updatedReport.destination) missing.push('Trecho');
      if (!updatedReport.start_date) missing.push('Data Início');
      if (missing.length) {
    ...
    try {
      if (!updatedReport.numero) {
        if (updatedReport.client_name) {
          try {
            updatedReport.numero = await allocateReportNumber(updatedReport.client_name);
          } catch {
            updatedReport.numero = generateReportNumberLocal(updatedReport.client_name);
          }
        } else {
    ...
    if (status !== 'finalized') {
      try {
        if (!updatedReport.numero && updatedReport.client_name) {
          try {
            updatedReport.numero = await allocateReportNumber(updatedReport.client_name);
          } catch {
            updatedReport.numero = generateReportNumberLocal(updatedReport.client_name);
    ...
    const folder = toFolderSafe(updatedReport.client_name);
    ...
    try {
      ...
      if (!finalNumber || finalNumber.startsWith('rascunho_')) {
        try {
          if (updatedReport.client_name) {
            finalNumber = await allocateReportNumber(updatedReport.client_name);
          }
        } catch {
          finalNumber = generateReportNumberLocal(updatedReport.client_name);
        }
      }

      const payload: any = {
        numero: finalNumber,
        client_name: updatedReport.client_name || '',
        aircraft_registration: updatedReport.aircraft_registration || '',
        crew_member_name: updatedReport.crew_member_name || '',
        crew_member_name_2: updatedReport.crew_member_name_2 || null,
        destination: updatedReport.destination || '',
        start_date: updatedReport.start_date || '',
        end_date: updatedReport.end_date || null,
        observations: updatedReport.observations || null,
        total_amount: Number(updatedReport.total_amount || 0),
        total_fuel: Number(updatedReport.total_fuel || 0),
        total_lodging: Number(updatedReport.total_lodging || 0),
        total_food: Number(updatedReport.total_food || 0),
        total_transport: Number(updatedReport.total_transport || 0),
        total_other: Number(updatedReport.total_other || 0),
        total_crew: Number(updatedReport.total_crew || 0),
        total_client: Number(updatedReport.total_client || 0),
        total_sharebrasil: Number(updatedReport.total_sharebrasil || 0),
        ...
        try {
          const ins = await (supabase as any)
            ...
            .insert({
              ...
              cliente: updatedReport.client_name,
              pdf_path: pdfPath,
              metadata: {
                status: 'submitted',
                destination: updatedReport.destination,
                ...
                totals: { total_amount: updatedReport.total_amount }
              }
            })
            .select('*')
            .single();
  ...
  const handleExpenseChange = (index, field, value) => {
    const newExpenses = [...currentReport.despesas];
    newExpenses[index][field] = field === 'valor' ? value : value;
    setCurrentReport(prev => ({ ...prev, despesas: newExpenses }));
  ...
  const removeExpense = (index) => {
    const newExpenses = currentReport.despesas.filter((_, i) => i !== index);
    setCurrentReport(prev => ({ ...prev, despesas: newExpenses }));
    calculateTotals(newExpenses);
  };
  ...
  const handleFileUpload = async (index, file) => {
    ...
    try {
      ...
      handleExpenseChange(index, 'comprovante_url', dataUrl);
    } catch (error) {
  ...
  const generateHTMLReport = (report: any) => {
    const calcDays = () => {
      if (report?.start_date && report?.end_date) {
        const inicio = parseLocalDate(report.start_date);
        const fim = parseLocalDate(report.end_date);
        const diffTime = Math.abs(fim.getTime() - inicio.getTime());
        const diffDays = Math.ceil(diffTime / (1000 * 60 * 60 * 24)) + 1;
        return diffDays;
      }
    ...
    <body>
        ...
        <div class="report-container">
            <div class="header">
                ...
                <div class="header-title">
                    ...
                    <div class="header-info">
                        <span>${formattedNumero} - ${report.client_name || 'N/A'}</span>
                    </div>
                </div>
            </div>
            ...
            <div class="info-section">
                <div class="info-grid">
                    <div class="info-item"><strong>Cliente:</strong> ${report.client_name || 'N/A'}</div>
                    <div class="info-item"><strong>Aeronave:</strong> ${report.aircraft_registration || 'N/A'}</div>
                    <div class="info-item"><strong>Tripulante:</strong> ${[report.crew_member_name, report.crew_member_name_2].filter(v => v && String(v).trim()).join(' e ') || 'N/A'}</div>
                    <div class="info-item"><strong>Trecho:</strong> ${report.destination || 'N/A'}</div>
                    <div class="info-item"><strong>Período:</strong> ${report.start_date ? formatDateBR(report.start_date) : 'N/A'} a ${report.end_date ? formatDateBR(report.end_date) : 'N/A'} (${calcDays()} dias)</div>
                </div>
                ${report.observations && String(report.observations).trim() ? `
                <div>
                    <strong>Observações:</strong>
                    <p style="margin-top: 5px; font-size: 13px;">${String(report.observations).trim()}</p>
                </div>
                ` : ''}
            </div>
            ...
            <div class="totals-grid">
                <div class="totals-box">
                    ...
                    <div class="total-row"><span>Combustível:</span> <span>${(report.total_fuel || 0).toFixed(2).replace('.', ',')}</span></div>
                    <div class="total-row"><span>Hospedagem:</span> <span>${(report.total_lodging || 0).toFixed(2).replace('.', ',')}</span></div>
                    <div class="total-row"><span>Alimentação:</span> <span>${(report.total_food || 0).toFixed(2).replace('.', ',')}</span></div>
                    <div class="total-row"><span>Transporte:</span> <span>${(report.total_transport || 0).toFixed(2).replace('.', ',')}</span></div>
                    <div class="total-row"><span>Outros:</span> <span>${(report.total_other || 0).toFixed(2).replace('.', ',')}</span></div>
                    <div class="total-final total-row"><span>TOTAL GERAL:</span> <span>${(report.total_amount || 0).toFixed(2).replace('.', ',')}</span></div>
                </div>
                <div class="totals-box">
                    <h3>Totais por Pagador (R$)</h3>
                    <div class="total-row"><span>Tripulante:</span> <span>${(report.total_crew || 0).toFixed(2).replace('.', ',')}</span></div>
                    <div class="total-row"><span>Cliente:</span> <span>${(report.total_client || 0).toFixed(2).replace('.', ',')}</span></div>
            ...
            <div class="signature-box">
                <div class="signature-line">Assinatura do Tripulante(s): ${[report.crew_member_name, report.crew_member_name_2].filter(v => v && String(v).trim()).join(' / ') || 'N/A'}</div>
                <div class="signature-line">Assinatura do Cliente: ${report.client_name || 'N/A'}</div>
            </div>
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="flex flex-col">
              ...
              <Select 
                value={currentReport.client_name || ''} 
                onValueChange={(value) => {
            ...
            <div className="flex flex-col">
              ...
              <Select 
                ...
                onValueChange={(value) => handleInputChange('aircraft_registration', value)}
              >
                <SelectTrigger>
            ...
            <div className="flex flex-col">
              <div className="flex items-center justify-between mb-1">
                ...
                {!showSecondCrew && (
                  <button
                    type="button"
                    onClick={() => setShowSecondCrew(true)}
              ...
              <Select
                ...
                onValueChange={(value) => {
                  if (value === OUTRO_OPTION) {
                    setIsOutroCrew(true);
                    handleInputChange('crew_member_name', '');
                  } else {
                    setIsOutroCrew(false);
                    handleInputChange('crew_member_name', value);
                  }
                }}
              >
                <SelectTrigger>
            ...
            {showSecondCrew && (
              <div className="flex flex-col">
                ...
                <Select
                  value={crew2SelectValue}
                  onValueChange={(value) => {
                    if (value === OUTRO_OPTION) {
                      setIsOutroCrew2(true);
                      handleInputChange('crew_member_name_2', '');
                    } else {
                      setIsOutroCrew2(false);
                      handleInputChange('crew_member_name_2', value);
                    }
                  }}
                >
                  <SelectTrigger>
            ...
            <div className="flex flex-col">
              ...
              <input
                type="text"
                value={currentReport.destination || ''}
            ...
            <div className="flex flex-col">
              ...
              <input
                type="date"
                value={currentReport.start_date || ''}
            ...
            {currentReport.despesas.map((despesa, index) => (
              <div key={index} className="border p-4 rounded-lg bg-gray-900 relative">
                <div className="absolute top-2 right-2 flex space-x-2">
                  <button onClick={() => removeExpense(index)} className="p-1 rounded-full text-red-500 hover:bg-red-900">
                    <Trash2 size={18} />
                  </button>
                </div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
                  <div className="flex flex-col">
                    ...
                    <Select 
                      ...
                      onValueChange={(value) => handleExpenseChange(index, 'categoria', value)}
                    >
                  ...
                  <div className="flex flex-col">
                    ...
                    <input
                      type="number"
                      value={despesa.valor || ''}
                      onChange={(e) => handleExpenseChange(index, 'valor', e.target.value)}
                      placeholder="0.00"
                  ...
                  <div className="flex flex-col">
                    ...
                    <Select 
                      ...
                      onValueChange={(value) => handleExpenseChange(index, 'pago_por', value)}
                    >
                      <SelectTrigger className="text-xs">
                        <SelectValue placeholder="Selecione" />
                      </SelectTrigger>
                ...
                <div className="flex flex-col">
                  ...
                  <input
                    ...
                    onChange={(e) => handleExpenseChange(index, 'descricao', e.target.value)}
          ...
          <button onClick={addExpense} className="w-full flex items-center justify-center space-x-2 p-2 border-2 border-dashed border-gray-600 text-gray-400 rounded-md hover:border-blue-400 hover:text-blue-400 transition">
            <Plus size={20} />
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="border p-4 rounded-lg bg-gray-900 shadow-sm border-gray-700">
              ...
              <div className="space-y-1 text-sm text-gray-200">
                <div className="flex justify-between"><span>Combustível:</span> <span className="font-semibold">R$ {currentReport.total_fuel.toFixed(2).replace('.', ',')}</span></div>
                <div className="flex justify-between"><span>Hospedagem:</span> <span className="font-semibold">R$ {currentReport.total_lodging.toFixed(2).replace('.', ',')}</span></div>
                <div className="flex justify-between"><span>Alimentação:</span> <span className="font-semibold">R$ {currentReport.total_food.toFixed(2).replace('.', ',')}</span></div>
                <div className="flex justify-between"><span>Transporte:</span> <span className="font-semibold">R$ {currentReport.total_transport.toFixed(2).replace('.', ',')}</span></div>
                <div className="flex justify-between"><span>Outros:</span> <span className="font-semibold">R$ {currentReport.total_other.toFixed(2).replace('.', ',')}</span></div>
                <div className="flex justify-between pt-2 border-t border-gray-700 font-bold text-lg text-green-400"><span>TOTAL GERAL:</span> <span>R$ {currentReport.total_amount.toFixed(2).replace('.', ',')}</span></div>
              </div>
            </div>
            <div className="border p-4 rounded-lg bg-gray-900 shadow-sm border-gray-700">
              <h3 className="text-md font-bold mb-2 text-blue-300">Totais por Pagador</h3>
              <div className="space-y-1 text-sm text-gray-200">
                <div className="flex justify-between"><span>Tripulante:</span> <span className="font-semibold">R$ {currentReport.total_crew.toFixed(2).replace('.', ',')}</span></div>
                <div className="flex justify-between"><span>Cliente:</span> <span className="font-semibold">R$ {currentReport.total_client.toFixed(2).replace('.', ',')}</span></div>
                <div className="flex justify-between"><span>ShareBrasil:</span> <span className="font-semibold">R$ {currentReport.total_sharebrasil.toFixed(2).replace('.', ',')}</span></div>
                <div className="flex justify-between pt-2 border-t border-gray-700 font-bold text-lg text-green-400"><span>TOTAL GERAL:</span> <span>R$ {currentReport.total_amount.toFixed(2).replace('.', ',')}</span></div>
              </div>
            </div>
          </div>
  ...
  // RENDERIZAÇÃO - Lista de Relatórios
  return (
    <Layout showRightSidebar={true}>
            ...
            {visibleReports.sort((a,b) => new Date(b.updatedAt).getTime() - new Date(a.updatedAt).getTime()).map((report) => (
              <div key={report.numero} className="flex items-center justify-between p-3 border rounded-lg hover:bg-gray-50">
                <div className="flex-1 min-w-0">
                  <p className="font-medium truncate">{report.client_name || 'Sem Cliente'}: {report.numero}</p>
                  <p className="text-sm text-gray-500 truncate">{report.destination || 'Trecho Não Informado'} - R$ {report.total_amount.toFixed(2).replace('.', ',')}</p>
